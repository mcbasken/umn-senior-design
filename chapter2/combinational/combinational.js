...
function setup() {
  ...
  for (let i = 0; i < inputBits.length; i++) {
    ...
    inputButtons[i].position(80 + i * 60, 60);
  ...
  switchBtn.style('text-align', 'center');
  switchBtn.style('font-size', '14px');
...
function basicGates() {
  ...
  text(`A AND B = ${A & B}`, width / 2, 150);
  text(`A OR B = ${A | B}`, width / 2, 180);
  text(`NOT A = ${A ^ 1}`, width / 2, 210);
  text(`A XOR B = ${A ^ B}`, width / 2, 240);
}
...
function halfAdder() {
  ...
  let sum = A ^ B;
  let carry = A & B;
  text(`A = ${A}, B = ${B}`, width / 2, 130);
  text(`Sum = ${sum}`, width / 2, 160);
  text(`Carry = ${carry}`, width / 2, 190);
}
...
function fullAdder() {
  ...
  text(`A = ${A}, B = ${B}, Cin = ${Cin}`, width / 2, 120);
  text(`Sum = ${sum}`, width / 2, 150);
  text(`Cout = ${carry}`, width / 2, 180);
}
...
function mux2to1() {
  ...
  let Sel = inputBits[3];
  let Y = Sel ? I1 : I0;
  text(`I0 = ${I0}, I1 = ${I1}, Sel = ${Sel}`, width / 2, 130);
  text(`Output Y = ${Y}`, width / 2, 160);
}
...
function decoder2to4() {
  ...
  let I1 = inputBits[1];
  let sel = (I1 << 1) | I0;
  text(`Inputs: I1 = ${I1}, I0 = ${I0}`, width / 2, 110);
  for (let i = 0; i < 4; i++) {
    text(`Y${i} = ${i === sel ? 1 : 0}`, width / 2, 140 + i * 20);
  }
}
...
function comparator() {
  ...
  text(`A = ${A} (${inputBits[0]}${inputBits[1]}), B = ${B} (${inputBits[2]}${inputBits[3]})`, width / 2, 110);
...
function alu4bit() {
  let A = (inputBits[0] << 2) | (inputBits[1] << 1);
  let B = (inputBits[2] << 1) | inputBits[3];
  let sum = A + B;
  let and = A & B;
  let or = A | B;
